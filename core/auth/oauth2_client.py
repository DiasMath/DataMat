...
class OAuth2Client:
    ...
    Esperado no .env:
      ...
      OAUTH_CLIENT_SECRET
      OAUTH_REDIRECT_URI
      OAUTH_SCOPE                (opcional)
      OAUTH_AUTH_CODE           (apenas no 1º bootstrap)
    """
    ...
    def _set_exp(self, tokens: Dict) -> Dict:
        ...
        tokens["expires_at"] = int(time.time()) + max(0, exp - 30)  # margem
        return tokens
    ...
    def exchange_code(self, authorization_code: Optional[str] = None) -> Dict:
        ...
        tokens = self._set_exp(self._post_token(payload))
    ...
    def refresh(self) -> Dict:
        ...
        if not self.use_basic_auth:
            payload.update({"client_id": self.client_id, "client_secret": self.client_secret})
        tokens = self._set_exp(self._post_token(payload))
        tokens.setdefault("refresh_token", rt)
        self._tokens = {**self._tokens, **tokens}
        self._save_tokens()
        return tokens
